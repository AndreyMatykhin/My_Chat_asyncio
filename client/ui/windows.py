...
class ContactsWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, client_instance=None, user_name=None, parent=None):
        super().__init__(parent)
...
class ChatWindow(QtWidgets.QMainWindow):
    ...
    def update_chat(self, quantity=20):
        ...
        client_msgs = [c for c in self.client_instance.get_client_messages(self.username) if
                       c.contact.username == self.contact_username]
        contact_msgs = [c for c in self.client_instance.get_client_messages(self.contact_username) if
                        c.contact.username == self.username]
        ...
        for msg in msgs[-quantity:]:
            ...
            if msg.client.username == self.username:
                sender = 'me'
            self.ui.chat_window.addItem(f'{msg.time.strftime("%Y-%m-%d %H:%M:%S")} from {sender}: {msg.message}')
